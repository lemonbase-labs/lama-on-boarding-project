...
class ReviewCycleDomainService:
    ...
    @classmethod
    def _is_changed_reviewee_set(cls, origin_reviewee: List[Reviewee], reviewee_ids: List[int]) -> bool:
        # TODO: 순서가 중요한지에 따라, 순서 체크시 로직을 추가한다
        origin_reviewee_person_ids = set([reviewee.person_id for reviewee in origin_reviewee])
    ...
    @classmethod
    @transaction.atomic
    def update_review_cycle(cls, review_cycle_update_command: ReviewCycleUpdateCommand) -> ReviewCycle:
        review_cycle: ReviewCycle = ReviewCycleRepository.find_one(review_cycle_update_command.review_cycle_id)

        if review_cycle.review_cycle_creator.id != review_cycle_update_command.request_user_id:
        ...
        review_cycle.question.description = review_cycle_update_command.question.description

        if cls._is_changed_reviewee_set(review_cycle.reviewee_set.all(), review_cycle_update_command.reviewee_ids):
